...
impl SqliteAssetRepository {
    ...
    pub async fn run_migration(&self) -> ArkResult<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| dark_core::ArkError::Internal(format!("Asset migration failed: {e}")))?;
        ...
        .await
        .map_err(|e| {
            dark_core::ArkError::Internal(format!("Asset issuance migration failed: {e}"))
        })?;
...
impl AssetRepository for SqliteAssetRepository {
    async fn store_asset(&self, asset: &Asset) -> ArkResult<()> {
        let metadata_json = serde_json::to_string(&asset.metadata)
            .map_err(|e| dark_core::ArkError::Internal(format!("Metadata serialization: {e}")))?;
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| dark_core::ArkError::Internal(format!("Store asset: {e}")))?;
    ...
    async fn get_asset(&self, asset_id: &str) -> ArkResult<Option<Asset>> {
        ...
        .await
        .map_err(|e| dark_core::ArkError::Internal(format!("Get asset: {e}")))?;
    ...
    async fn list_assets(&self) -> ArkResult<Vec<Asset>> {
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| dark_core::ArkError::Internal(format!("List assets: {e}")))?;
    ...
    async fn store_issuance(&self, issuance: &AssetIssuance) -> ArkResult<()> {
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| dark_core::ArkError::Internal(format!("Store issuance: {e}")))?;
