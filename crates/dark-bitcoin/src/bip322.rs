...
//! Bitcoin address by signing a message. Used by dark to verify that intent
...
impl Bip322Proof {
    ...
    ///
    /// The expected message is `"dark intent: {intent_id}"`. The proof must
...
///
/// Only supports key-path Taproot spend for now (the dominant case for dark
...
fn verify_simple_witness(
    ...
    // Only P2TR (Taproot key-path) is supported. dark identifies VTXO owners
...
mod tests {
    ...
    fn test_format_intent_message() {
        assert_eq!(format_intent_message("abc-123"), "dark intent: abc-123");
    }
