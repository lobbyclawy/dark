...
//!
//! Aligns with Go dark port interfaces.
...
pub trait FeeManagerService: Send + Sync {
    ...
    ///
    /// Mirrors Go dark's `FeeManager.ComputeIntentFees`.
...
mod tests {
    ...
    async fn test_env_unlocker_missing_var() {
        ...
        let err_msg = format!("{}", result.unwrap_err());
        assert!(
            err_msg.contains("DARK_WALLET_PASS"),
...
/// the Go dark conviction system with rich querying capabilities.
#[async_trait]
...
///
/// Mirrors Go dark's `TxDecoder` interface.
#[async_trait]
...
///
/// Mirrors Go dark's `Unlocker` interface.
#[async_trait]
...
/// Environment-variable-based unlocker — reads `DARK_WALLET_PASS`.
pub struct EnvUnlocker;

#[async_trait]
impl Unlocker for EnvUnlocker {
    async fn get_password(&self) -> ArkResult<String> {
        std::env::var("DARK_WALLET_PASS").map_err(|_| {
            crate::error::ArkError::Internal("DARK_WALLET_PASS environment variable not set".into())
        })
    }
}
...
///
/// Mirrors Go dark's `BatchFinalizedAlert` struct.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
///
/// Mirrors Go dark's `Alerts` interface.
#[async_trait]
...
///
/// Mirrors Go dark's event-store pattern (Badger/Postgres backed).
