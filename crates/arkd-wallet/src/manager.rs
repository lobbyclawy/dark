...
impl WalletManager {
    ...
    pub async fn new(config: WalletConfig) -> WalletResult<Self> {
        ...
        let mut db = FileStore::open_or_create_new(b"arkd-wallet", &db_path).map_err(|e| {
    ...
    fn persist_wallet_static(
        ...
        let mut db = FileStore::open_or_create_new(b"arkd-wallet", db_path)
...
mod tests {
    ...
    async fn test_wallet_manager_creation_with_mnemonic() {
        let config = WalletConfig::regtest("/tmp/arkd-wallet-test").with_mnemonic(
