//! `TxDecoder` trait implementation for `arkd_bitcoin::BitcoinTxDecoder`.
//!
//! Bridges the standalone `BitcoinTxDecoder` from `arkd-bitcoin` to the
//! `TxDecoder` trait defined in `arkd-core::ports`.
...
use arkd_bitcoin::tx_decoder::BitcoinTxDecoder;
